...
def prepare_vector_preflight(
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=min(max_workers, len(inputs["slides"]))) as pool:
...
def _load_inputs(root: Path) -> dict[str, Any]:
    ...
    _validate_batch(root, batch, request["slides"])
...
def _run_measurement_batches(
    *,
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        batch_path = write_json(batch_root / f"batch-{index:02d}.json", batch_payload)
        command = [
            executable,
